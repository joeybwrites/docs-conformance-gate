...
**Part 4 — Adoption DRAFTED** (`adoption/part4_adoption.md`). AI-assisted first draft, finalized in Joey's voice. Grounded in a real prior experience (a docs-quality tool that failed by being optional and succeeded where a producing team's lead enforced it): useful-first + shift-left + a publish chokepoint docs controls + tiered, defensible enforcement.

## Disclosure reconciliation (closing)

- **AI use.** Primary build surface: Claude, via Claude Code. The workflow also used **multiple parallel Claude sub-agents** — two reference sweeps across the estate that fed the matrix and drift evidence, one Part-4 adoption first-draft, and the reconciliation-pass edits to the standard, matrix, overview, and Part 4 — all Claude, all disclosed here by material contribution. **ChatGPT / Codex (OpenAI)** contributed only the off-clock Session-1 orientation and checker-approach research (a stability fallback), as named in Session 1; it produced no shipped deliverable content. Joey directed and reviewed throughout, and finalized the outward prose (memo, before/after, Part 4) in his own voice.
...
- **State at stop.** Parts 1–4 are complete: the analysis, standard, and checker are settled, and the outward prose (memo, before/after, Part 4) is finalized in Joey's voice. The Part 3 checker is verified end-to-end (all fixtures + the `test_gate.py` regression suite + the 5-page corpus + JSON + exit codes). Known checker limitations are documented in `checker/README.md`. The earlier standard-vs-deliverable tension (strict first-mention S4 vs. the AFTER's prose ordering) was resolved in the reconciliation pass: the before/after now links each primitive at first mention, so it conforms to its own S4. All of it is committed and pushed to `main`.
