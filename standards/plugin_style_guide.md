...
## A. Style rules (each: rule · problem it prevents · proposed enforcement)

| # | Rule | Prevents | Proposed enforcement (Part 3) |
|---|------|----------|------------------------------|
| **S1** | **Canonical definition, single owner.** Each core concept (plugin, skill, connector/MCP, slash command, sub-agent, hook) has one canonical OWNER for each registered definition or claim; other pages may give bounded contextual summaries and link to that owner. The rule forbids a second source of truth, not a contextual restatement. | P2 drift | *(proposed / partly human)* A definition phrase duplicated verbatim on a non-owner page flags for review; whether a restatement is a bounded summary or a competing definition is a human call. |
| **S2** | **Surface-scoped claims, matrix-backed.** A statement of what a plugin contains or where it runs names the surface or references the matrix, and matches the matrix's evidenced state for that surface. | P1 two-model collision | *(proposed / partly human)* Component enumeration vs the matrix row for the named surface; the automated part checks the enumeration against the row, a reviewer confirms the surface was correctly identified. |
| **S3** | **Single availability source, dated.** Availability/lifecycle status has one owner; no page contradicts it; no undated relative-time promises ("coming soon", "in the weeks ahead"). | P3 contradiction, P5 | *(implemented proxy — owner-decision)* Relative-time regex in availability sections is implemented; the owner-vs-page contradiction check runs as a proxy that surfaces candidate contradictions for an owner decision rather than settling them. |
| **S4** | **Prerequisite bridging.** Every core concept a page uses that is not in its `assumes:` frontmatter links to its owning page on first mention. | P4 / JF1 | *(implemented automated check)* For each registry concept mentioned and not in `assumes`, require a first-mention link. |
| **S5** | **Contextualized cross-property handoff.** Crossing from `claude.com/docs` to `code.claude.com` or `support.claude.com` is legitimate, not a failure. A handoff for a decision-critical step must (a) summarize the decision-critical information the reader needs, (b) state why the destination owns that step, and (c) link to the precise section. A dedicated task page may be linked at its root when the ownership registry identifies the whole page as the destination. **Flag unframed, premature, or ambiguous handoffs.** This is a **partly semantic** requirement: an automated check can confirm the structural signals (adjacent summary, ownership sentence, anchor) but cannot prove the summary is *sufficient* or the framing *apt* — it is a useful proxy, not full compliance. | P4 offloading (correctly scoped) | *(partial / proxy)* An off-property decision link must carry an adjacent summary and ownership sentence; require a `#anchor` unless the target URL matches a dedicated task page in the ownership registry; an unregistered bare root or unframed link fails. Adequacy of the summary is left to review. |
| **S6** | **Canonical link form + terminology.** No doubled-prefix (`/docs/docs/…`) or alias link forms; terminology follows the canonical spelling that a named terminology owner or the registry declares. The canonical spelling of a contested term (e.g. sub-agent / subagent / agent) is whatever the registry records once the product decision is made — the registry is authoritative, not this document's guess, and the term denylist is generated from it. | P5 | *(implemented automated check)* Doubled-prefix / alias regex is implemented; the terminology denylist is checked automatically against whatever the registry currently declares canonical. |
| **S7** | **Role-specific required sections.** A page's required sections are keyed to its owned role. **Creation page:** package anatomy, implementation steps, testing, debugging. **Submission page:** readiness, eligibility, required inputs, validation's relationship to review, review states, decision points, publication outcomes, exception recovery. **Lifecycle-map (overview) page:** definition, platform relationship, matrix, availability, surface-use orientation, lifecycle map. The orientation gives readers a concrete first-use path without duplicating the dedicated procedure owner. *A submission page must not duplicate the authoring guide's package tree or build procedure.* | JF2, fragmentation | *(proposed / partly human)* Required-heading presence keyed to the page's declared `content-type` / role is machine-checkable; whether a present section actually discharges its role is a human call. |
| **S8** | **Owned procedures are executable.** For the actual procedures and consequential transitions a page owns (per S7) — submit, publish, and other irreversible or user-decision steps — each carries explicit prerequisites, an expected outcome, validation/evidence of success, failure handling, and marked decision/stop points, so an agent stops and asks and a human doesn't miss them. This applies to consequential steps, not to every trivial numbered item; a plain sequential instruction is not required to carry a prerequisites/outcome/validation block. This is a **partly semantic** requirement: an automated check can flag a consequential step missing its block or stop point, but cannot prove a present block is *correct or complete* — it is a useful proxy, not full compliance. | high-consequence safety, agentic-usability | *(proposed / partly human)* A submit/publish (or otherwise marked irreversible) step missing a prerequisites/outcome/validation block or a marked stop point flags; classifying which steps are consequential, and judging block correctness, is left to review. |
